# TradeTrackerJournal
 Purpose: 
 - I am building this simple long-only trading journal to learn & practice react + microservices architecture.
 What I learned: I learned how to create HTTP RESTful APIs using Javascript to create, read, update, and delete trade entries in the back-end communication with the MongoDB database and relay the reflected changes to the frontend. I also am learning git, jsonwebtoken authentication, react components, and how to utilize and implement another developer's microservice using ZeroMQ connecting to a server. This project is currently still in-progress as I am continuously building upon technical skills that I have learned.

Intended Use Case:
- For users who want to track their trades and see their profit and loss performance over time.

Features:
- Simple signup, login, logout authentication with JsonWebTokens
- Intuitive Add, Edit, and Delete trade entries
- User trading perfomance is shown as a line graph using Recharts library
- Simple trading data table
- Simple user interface

Currently working on:
- Creating an aesthetic frontend component to showcase the returned data from ZeroMQ microservice as shown in the screenshot below "Processed Data".
- Integrating more statistics for trades
- Integrating a notes section for users to journal their thoughts or strategies.

How to get started:
- Clone the repository
- install dependencies
- npm run dev the backend and frontend in separate terminals.


Screenshot of current work-in-progress: 
![image](https://github.com/user-attachments/assets/b5e8bf52-4dd7-4c23-8d82-f24169574fa9)

